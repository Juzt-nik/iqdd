...
An image-based full-stack application that takes an image and analyzes its visual quality, including detection of blurriness, under/overexposure, noise, corruption, and localized visual defects. Built using a hybrid classical CV + deep-learning approach without the use of external AI/vision APIs.

**Live deployment:**
- Frontend: https://iqdd.vercel.app
- Backend API: https://iqdd.onrender.com 
- Backend Health check: https://iqdd.onrender.com/health -> {"status": "ok", "models_loaded": {"tree": true, "cnn": true, "autoencoder": true}, "database": "ok"}

> Both are current deployed on free-tier hosting. Limitation: The backend sleeps after 15 minutes of inactivity —> the first request is idle, may take time around 30-60 seconds to
> respond while the backend wakes up. 
...
Source images are 3,600 real photos from Kaggle's [`prasunroy/natural-images`](https://www.kaggle.com/datasets/prasunroy/natural-images)
...
│   │   └── train_cnn.py
│   ├── data/
│   │   ├── clean/             ← Clean source images are put here
...
## Quickstart — Docker (recommended for faster evaluation)
...
```bash
cd backend
py -3.11 -m venv venv (put due to the use of cv2)
source venv/bin/activate
pip install -r requirements.txt
uvicorn app.main:app --reload --port 8000

//i.e cv2 is incompatible with higher py --version
```
...
## Bonus Work CheckList

| Bonus Item | Status |
|---|---|
| **Batch image analysis** | ✅ **Present** — `/analyze` takes more than one file at a time. |
| **Quality heatmaps / localization** | ✅ **Present** — Grad-CAM + autoencoder reconstruction map, with `heatmap_png_base64` in every response. |
| **Confidence calibration / uncertainty estimates** | The autoencoder's z-score against clean-validation stats is a form of calibration, but it is not a general uncertainty estimate across all issue types, and it is not labeled/discussed as such because we claim it for what it is. |
| **Model versioning** | `model_versions` in the API response is just booleans for whether each model loaded, not actual version numbers or a model registry. |
| **Performance** | No explicit async inference handling, no optimization for concurrent requests, and no worker-pool tuning beyond FastAPI's defaults. |
| **Monitoring / logging** | Basic Python `logging` exists in `main.py` (startup, model load errors), but it is minimal application logging, not real monitoring (no request metrics, error tracking service, or dashboards). |
